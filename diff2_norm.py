...
# 对归一化一阶差分进行差分，生成对应的二阶差分值
y_diff2_norm = np.diff(y_diff_norm)

# 将末尾差分值直接设为0，以防止漏掉尾峰
y_diff2_norm[-1] = 0

# 头点特别处理，直接将差分尾点Q[0] = D[0],即认为头点S[n]之前保持不变，以免漏掉头峰
y_diff2_norm[0] = y_diff_norm[0]

# 差分后的横坐标数组
x_diff2 = x[:-2]

trace = go.Scatter(x=x_diff2, y=y_diff2_norm, mode='lines', name='Normalization 2nd Difference')
data = [trace]
layout = go.Layout(title='Normalization 2nd Difference Plot')
fig = go.Figure(data=data, layout=layout)
pyo.plot(fig, filename='my_plot_diff2_norm.html')
