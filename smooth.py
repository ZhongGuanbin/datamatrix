...
# # 采用Butterworth滤波法，平滑曲线
# y_smooth = savgol_filter(y_sub, 31, 3)

# Butterworth滤波器，将截止频率设为50
cutoff = 150
nyquist_rate = 0.5 * 400
cutoff_norm = cutoff / nyquist_rate
b, a = butter(3, cutoff_norm, btype='lowpass')
y_smooth = filtfilt(b, a, y_sub)

trace = go.Scatter(x=x, y=y_smooth, mode='lines')
data = [trace]
layout = go.Layout(title='Interactive Plot')
fig = go.Figure(data=data, layout=layout)
pyo.plot(fig, filename='my_plot_smooth_butter_150_3.html')
