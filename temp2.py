...
ax.plot(np.array(test_data[109]), color='black', linewidth=2, label='白色格子灰度趋势')
ax.plot(np.array(test_data[11]), color='black', linewidth=2, label='黑色格子灰度趋势', marker='^')
ax.set_xlim([0, max(len(np.array(test_data[109])), len(np.array(test_data[11])))])
ax.set_ylim([0, 320])
ax.set_xlabel('网格划分次数', fontsize=12)
ax.set_ylabel('网格灰度均值', fontsize=12)
ax.set_title('失真导致边缘凹凸、模糊情况下网格灰度均值变化趋势', fontsize=16)
ax.legend(loc='upper right', fontsize=10)
ax.tick_params(labelsize=10)

plt.show()
fig.savefig('grey_tend_5.png', dpi=500)
