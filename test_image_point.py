...
def find_peak(y_input):
    ...
    # 因为点状码存在较多缝隙，相当于多了很多行和列，所以要扣除这些长度，所以阈值需要减小
    y_sub = np.where(y_input < 5000, 0, y_input)
    ...
    for i in range(len(peak_list_find_peak) - 1):
        ...
        # 因为点状码存在较多缝隙，相当于多了很多行和列，所以要扣除这些长度，所以阈值需要减小
        if x_diff <= max(2, prediction_peak_spacing / 3):
...
def decode_dmtx(image_path):
    ...
    # 得到网格划分后每个网格的四个顶点坐标
    grid_points_list = grid_division(peak_list_x=find_peak_list_x, peak_list_y=find_peak_list_y, is_point=True)
